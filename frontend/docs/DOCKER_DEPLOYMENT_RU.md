...
**«Агент запустил сайт (например `http://localhost:8099/...`), но он не открывается».**
Из контейнера наружу по умолчанию проброшен только порт `8000`. Когда агент запускает
веб-сервер на `8099` внутри контейнера, с хоста он недоступен. В `docker-compose.yml` мы
добавили проброс диапазона `8090-8100`, поэтому:
- пересобери: `docker compose up -d --build`;
- теперь сайт агента на `8099` открывается как `http://localhost:8099/...`.
